...
SET @insertqiep = 'INSERT x_LEGACYDOC.AllDocs_LOCAL (DocumentRefID,DocumentType,DocumentDate,StudentRefID,StudentLocalID,MimeType,Content)
SELECT
...
WHERE ad.RecNum = (
 ...
 WHERE ad.GStudentID = y.GStudentID AND y.RecNum = ad.RecNum
 )'
 
SET @insertqph = 'INSERT x_LEGACYDOC.AllDocs_LOCAL (DocumentRefID,DocumentType,DocumentDate,StudentRefID,StudentLocalID,MimeType,Content) 
SELECT   DocumentRefID = ph.RecNum,  DocumentType = CAST(''PlacementHistory'' AS varchar(100)),   DocumentDate = PlaceDate,   StudentRefID = x.GStudentID,   StudentLocalID =  st.StudentID,   MimeType = CAST(''document/pdf'' AS varchar(25)),   Content = ph.SavedPDF  
FROM '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].SpecialEdStudentsAndIEPs x   
JOIN '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].Student st on st.GStudentID = x.GStudentID  
JOIN '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].SC_PlaceHistoryTbl ph ON x.GStudentID = ph.GstudentID  
WHERE ph.SavedPDF IS NOT NULL
AND ph.RecNum = (
 SELECT MAX(y.RecNum)
 FROM '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].SC_PlaceHistoryTbl y 
 WHERE ph.GStudentID = y.GStudentID
 )'
 

SET @insertqco = 'INSERT x_LEGACYDOC.AllDocs_LOCAL (DocumentRefID,DocumentType,DocumentDate,StudentRefID,StudentLocalID,MimeType,Content) 
SELECT DocumentRefID = co.COSSeqNum,  DocumentType = CAST(''COSF'' AS varchar(100)),   DocumentDate = co.ServicesEntrydate,   StudentRefID = x.GStudentID,   StudentLocalID =  st.StudentID,   MimeType = CAST(''document/pdf'' AS varchar(25)),   Content = co.SavedPDF  
FROM '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].SpecialEdStudentsAndIEPs x   
JOIN '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].Student st on st.GStudentID = x.GStudentID  
JOIN '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].COSTbl co ON x.GStudentID = co.GstudentID  
WHERE co.SavedPDF IS NOT NULL
AND co.COSSeqNum = (
 SELECT MAX(y.COSSeqNum)
 FROM '+isnull(@LinkedserverAddress,'linkservhere')+'.'+isnull(@DatabaseName,'dbnamehere')+'.[dbo].COSTbl y 
 WHERE co.GStudentID = y.GStudentID
 )'
